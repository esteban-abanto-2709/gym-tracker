...
`Routine` also carries `userId` and is unique per `[userId, name]` (so two users can each have a "Push"). Each `RoutineItem` (a slot: one exercise at a `position`) stores its targets in `blocks`, a JSON list of typed blocks discriminated by `kind` (today only `legacy`: `{ sets, reps, durationSec, approx }`, where `Exercise.isTimed` still decides time vs. weight — see TD-018). Block types live in `apps/api/src/modules/routines/blocks.ts` (validated by `dto/routine-block.dto.ts`) and `apps/web/src/lib/types.ts`; the web flattens them into planned sets in `apps/web/src/lib/blocks.ts`. Some fields on `Workout`/`Routine` (e.g. `routineId`, `isApproximation`) are omitted here for brevity — see `apps/api/prisma/schema.prisma`.
