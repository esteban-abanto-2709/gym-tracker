...
`Exercise.isTimed` + `Workout.durationSec` + `RoutineItem.targetDurationSec`, con `Workout.weight` nullable: un set trae peso **o** duración (`@ValidateIf` en el DTO lo obliga) y la recomendación nunca sugiere peso sin peso previo. El modo guiado pide segundos, el historial pinta "45 s" y el editor de rutinas fija metas en segundos; la casilla del modal marca un ejercicio como de tiempo al crearlo. Aplicado en dev y prod.
