...
## [RM-028] Ejercicios medidos por tiempo (2026-09-07 09:05)
`Exercise.isTimed` + `Workout.durationSec` + `RoutineItem.targetDurationSec`, con `Workout.weight` nullable: un set trae peso **o** duración (`@ValidateIf` en el DTO lo obliga) y la recomendación nunca sugiere peso sin peso previo. El modo guiado y el día libre piden segundos, el historial pinta "45 s" y deja editarlos, y el editor de rutinas fija metas en segundos; la casilla del modal marca un ejercicio como de tiempo al crearlo. Aplicado en dev y prod.
...
Dominio propio delegado a Cloudflare y named tunnel en lugar del quick tunnel: la URL pública queda fija con HTTPS y ya no cambia en cada reinicio. El dominio y el `TUNNEL_TOKEN` los pone cada quien en su `.env`, nunca el repositorio. Cierra el pendiente de RM-012.
