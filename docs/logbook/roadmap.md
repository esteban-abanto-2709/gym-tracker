...
- **Nota de implementación:** el default "último equipo del ejercicio" se resolvió con una **memoria en localStorage por ejercicio** (`gymtrack-equipment-by-exercise`), no consultando la BD. Cubre el caso principal (incluye sobrevivir refresh en el celular), pero es por-navegador. El respaldo por BD (cross-device) es RM-027.
