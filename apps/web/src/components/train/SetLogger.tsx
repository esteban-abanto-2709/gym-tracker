...
export function SetLogger({ item, equipment, logging, onLog }: SetLoggerProps) {
  const isTimed = item.exercise.isTimed ?? false;
  ...
  useEffect(() => {
    ...
    setSeconds(item.targetDurationSec?.toString() ?? "");
      ...
      {isTimed ? (
        <div className="space-y-2">
          <label className="kicker text-muted-foreground text-[0.6rem]">
            Segundos
            {item.targetDurationSec ? ` · meta ${item.targetDurationSec}` : ""}
          </label>
          <input
            type="number"
            inputMode="numeric"
